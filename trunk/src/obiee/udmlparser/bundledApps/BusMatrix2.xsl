...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
	...
	<xsl:for-each select="//BusinessModel/@id">
	<BusinessCatalog>
		<xsl:copy-of select="."/>
		<xsl:variable name="BMMLID" select="."/>
		<!-- copying all subject areas using this business model as source -->
		<PresentationCatalogIDList>
			<xsl:for-each select="../..//PresentationCatalog/RefBusinessModel/[@id = $BMMLID]">
				<PresentationCatalogID><xsl:value-of select="../@name"/></PresentationCatalogID>
			</xsl:for-each>
		</PresentationCatalogIDList>
